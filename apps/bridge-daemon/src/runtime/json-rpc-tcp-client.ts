...
export class JsonRpcTcpClient {
  ...
  async request<TResponse>(method: string, params?: unknown): Promise<TResponse> {
    ...
    if (!this.socket) {
      throw new Error("codex runtime tcp proxy is not available");
    }
  ...
  private async connectAndInitialize(): Promise<void> {
    await new Promise<void>((resolve, reject) => {
      const socket = createConnection({
        host: this.config.codexRuntimeProxyHost,
        port: this.config.codexRuntimeProxyPort,
      });
      let connected = false;
      ...
      socket.once("connect", () => {
        ...
        socket.on("error", (error) => {
          this.logger.warn("codex runtime tcp proxy error", error);
        });
        socket.once("close", () => {
          this.handleClose(new Error("codex runtime tcp proxy connection closed"));
        });
  ...
  private handleLine(line: string): void {
    ...
    try {
      ...
      this.logger.warn("failed to parse codex runtime tcp proxy message", {
        line,
        error,
      });
      return;
